...
int msgbuilder_enqueue(struct ctem_comm_handler *handler, void *data,
  ...
  for (unsigned chunk_idx = 0; sz > 4; chunk_idx++) {
    ...
    case MAGIC_PACKET: {
      ...
      if (len + offsetof(struct can2eth_can_chunk, data) != chunk_size) {
        netdev_warn(dev, "%s: can chunk size is unexpected (16+%u / %u)\n",
      ...
      if (can_chunk->flags) {
        ...
        if (!skb) {
          netdev_err(dev, "%s: Failed to allocate ksb for CANFD frame\n",
                     MODULE_NAME);
          return -ENOMEM;
        }
        ...
        if (!skb) {
          netdev_err(dev, "%s: Failed to allocate ksb for CAN frame\n",
                     MODULE_NAME);
          return -ENOMEM;
        }
        frame->can_id = can_id;
...
static __init int ctem_init_module(void) {
  ...
  for (registered_idx = 0; registered_idx < NUM_INTERFACES; registered_idx++) {
