...
int
msgbuilder_flush_if_it_is_time(struct ctem_comm_handler *handler) {
    pktbuilder_t *pkt_builder = handler->pktbuilder;
...
int
msgbuilder_enqueue(struct ctem_comm_handler *handler, void *data, uint16_t len,
                   uint16_t ctype) {
...
int
msgbuilder_init(struct ctem_comm_handler *handler) {
...
int
ctem_setup_udp(struct ctem_comm_handler *handler, int src_port) {
...
ctem_setup_communications(struct ctem_comm_handler *handler, int src_port,
    ...
    handler->workqueue = create_workqueue("ctem_rx_workqueue");
...
void
ctem_teardown_communications(struct ctem_comm_handler *handler) {
...
void
ctem_start_communications(struct ctem_comm_handler *handler) {
...
ctem_xmit(struct sk_buff *skb, struct net_device *dev) {
    ...
    return msgbuilder_enqueue(ctem_communications, &frame,
